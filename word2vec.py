...
def naiveSoftmaxLossAndGradient(
    ...
    #print(outsideWordIdx.shape)
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1
    gradCenterVec = np.dot(outsideVectors.T,y_hat-y)
    #print(outsideVectors.T.shape)
    #print(centerWordVec.shape)
    #print((y_hat-y).transpose(1,0))
    #gradOutsideVecs = np.dot(centerWordVec,(y_hat-y)[:,np.newaxis])
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(y_hat)
    #for k in range(K):
    #    loss += -np.log(sigmoid(-np.inner(centerWordVec, outsideVectors[negSampleWordIndices[k]])))

    for k in range(K):
        yn_hat = sigmoid(-np.dot(outsideVectors[indices[k + 1]], centerWordVec))
        loss += -np.log(yn_hat)
        gradCenterVec += np.dot(1-yn_hat, outsideVectors[indices[k + 1]])
        gradOutsideVecs[indices[k + 1]] += -np.dot(yn_hat - 1, centerWordVec)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        step_loss, gradCenterVec, gradOutsideVector = word2vecLossAndGradient(centerWordVec,
                                                                              outsideWordIdx,
                                                                              outsideVectors,
                                                                              dataset)
        loss += step_loss
        gradCenterVecs[centerWordId] += gradCenterVec
